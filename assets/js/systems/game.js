...
const Game = {
  ...
  showStatus() {
    ...
    Msg.info(`功率: ${Player.budget.powerUsed.toFixed(1)}/${Player.budget.powerMax}kW  算力: ${Player.budget.computeUsed.toFixed(1)}/${Player.budget.computeMax}MFlops`);
    Msg.info(`装备舱: ${Player.budget.bayUsed.toFixed(2)}/${Player.budget.bayMax}m³`);
    Msg.info(`经验: ${Player.exp}/${Player.expToNext}`);

    const slotLabels = {
      primary:'[武器/外部/弹药管道/数据/电源/界面]', secondary:'[武器/外部/弹药管道/数据/电源/界面]',
      armor:'[装甲/外部/界面]',
      ew1:'[电子战/外部/数据/电源/界面]', ew2:'[电子战/外部/数据/电源/界面]',
      generator:'[生成器/离子管道/电源/数据/界面]',
      container1:'[容器/内部/电源/界面]', container2:'[容器/内部/电源/界面]',
      repairer:'[修复器/外部/数据/电源/界面]',
      coreComputer:'[数据/界面]', corePower:'[电源/界面]'
    };
    for (const [slot, label] of Object.entries(slotLabels)) {
      ...
      if (e) {
        ...
        if (e.coreOutput) stats.push(e.coreType === 'computer' ? `算力+${e.coreOutput}` : `功率+${e.coreOutput}`);
        const extra = stats.length ? ` [${stats.join(' ')}]` : '';
