...
const CommandRegistry = {
  // 全局指令：任何场景下均可执行
  global: {
    help: { cmd: 'help', aliases: ['帮助', '?', 'h'], handler: 'cmdHelp', desc: '查看帮助', args: '[分类]' },
    save: { cmd: 'save', aliases: ['存档'], handler: 'cmdSave', desc: '保存游戏进度' },
    load: { cmd: 'load', aliases: ['读档'], handler: 'cmdLoad', desc: '读取游戏存档' },
    status: { cmd: 'status', aliases: ['状态', 'sta', 'sc', 'score', 'stats', 'st', '统计'], handler: 'cmdStatus', desc: '查看机体状态/任务统计' },
    bag: { cmd: 'bag', aliases: ['背包', 'inv', 'i'], handler: 'cmdBag', desc: '查看背包', args: '[-d]' },
    ...
    skills: { cmd: 'skills', aliases: ['技能', 'sk'], handler: 'cmdSkills', desc: '查看技能列表' },
    ...
    map: { cmd: 'map', aliases: ['地图'], handler: 'cmdMap', desc: '查看区域地图' },
  ...
  // 基地服务层指令：仅在安全区场景可用
  base: {
    shop: { cmd: 'shop', aliases: ['商店', 'buy', '购买'], handler: 'cmdShop', desc: '商店（装备库）', args: '[物品名|all]' },
    sell: { cmd: 'sell', aliases: ['出售'], handler: 'cmdSell', desc: '出售物品', args: '<物品名>' },
    trade: { cmd: 'trade', aliases: ['交易'], handler: 'cmdTrade', desc: '交易中心（供需/市场订单）', args: '[物资名称]' },
    '工业': { cmd: '工业', aliases: ['industry'], handler: 'cmdIndustry', desc: '查看工业区/设施' },
    ...
    '仓库': { cmd: '仓库', aliases: ['warehouse', 'wh'], handler: 'cmdWarehouse', desc: '仓库管理' },
    '机库': { cmd: '机库', aliases: ['hangar', 'hg'], handler: 'cmdHangar', desc: '机库/载具管理' },
    '任务': { cmd: '任务', aliases: ['quest'], handler: 'cmdQuest', desc: '任务接取/交付' },
