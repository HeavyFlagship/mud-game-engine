...
- [ ] **CHECK-SCENE-1**: 矿脉房间可拾取矿石资源
- [ ] **CHECK-SCENE-2**: 残骸房间可拾取机械零件
- [ ] **CHECK-SCENE-3**: 电磁干扰区视野减半
- [ ] **CHECK-SCENE-4**: 酸液池经过时扣装甲值
- [ ] **CHECK-SCENE-5**: 毒雾区每秒扣结构值
- [ ] **CHECK-SCENE-6**: 不同区域房间描述有差异化文本

---

## Boss 战斗验证

- [ ] **CHECK-BOSS-1**: 巨型守卫虫属性正确（HP 500，装甲 150，伤害 45）
- [ ] **CHECK-BOSS-2**: 震地冲击技能效果正确（范围 200m，伤害 30+减速 3 秒）
- [ ] **CHECK-BOSS-3**: 巨型守卫虫二阶段触发正确（HP<50% 攻速+30%，伤害+20%）
- [ ] **CHECK-BOSS-4**: 守护者巨像属性正确（HP 800，装甲 200，伤害 60）
- [ ] **CHECK-BOSS-5**: 护盾再生技能效果正确（每 30 秒恢复 100 护盾）
- [ ] **CHECK-BOSS-6**: 守护者巨像二阶段触发正确（高能模式，离子伤害）
- [ ] **CHECK-BOSS-7**: Boss 掉落独特装备和稀有材料

---

## 任务系统验证

- [ ] **CHECK-QUEST-1**: 任务数据结构正确（ID、名称、描述、目标、奖励、状态）
- [ ] **CHECK-QUEST-2**: 主线任务 1-3 可接取、追踪、完成
- [ ] **CHECK-QUEST-3**: 支线任务可重复接取
- [ ] **CHECK-QUEST-4**: 任务奖励正确发放（信用点、经验值、物品）
- [ ] **CHECK-QUEST-5**: `任务` 指令正确显示任务列表

---

## UI/UX 验证

- [ ] **CHECK-UI-1**: 战斗日志分类显示（攻击/移动/系统消息分颜色）
- [ ] **CHECK-UI-2**: 迷你地图显示已探索/未探索房间
- [ ] **CHECK-UI-3**: 迷你地图标记 Boss 房间
- [ ] **CHECK-UI-4**: 商店中装备属性与当前装备对比显示
...
- [ ] **CHECK-TEST-1**: 完整流程无报错：基地→探索→战斗→制造→交易→Boss→返回
- [ ] **CHECK-TEST-2**: 制造系统全流程正常：安装→运行→产出→解锁→安装新设施
- [ ] **CHECK-TEST-3**: 经济系统全流程正常：交付物资→供需变化→价格波动→配额补给
- [ ] **CHECK-TEST-4**: 边界场景不崩溃：背包满、弹药耗尽、机体损毁、面积不足
- [ ] **CHECK-TEST-5**: 存档/读档后制造状态一致
- [ ] **CHECK-TEST-6**: 存档/读档后经济状态一致
- [ ] **CHECK-TEST-7**: 10x10 地图加载时间 < 1 秒
- [ ] **CHECK-TEST-8**: 游戏可正常启动并显示开场介绍