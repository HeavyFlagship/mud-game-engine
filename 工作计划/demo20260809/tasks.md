...
- [x] **TASK-BAL-1**: 敌人难度曲线调整
  - [x] 荒原(wasteland): 工虫/突击虫 1-2只 → 荒野(wilderness): +飞虫/跳虫 1-3只 → 矿洞(cave): 喷酸虫/甲壳虫/毒雾虫 2-3只 → 机械遗迹(mech): 探针/防御节点/哨兵/守卫 1-3只 → 前沿(frontier): 重力扭曲器/防御节点/粒子束哨兵 2-3只
  - [x] Boss房间配置了随从敌人

- [x] **TASK-BAL-2**: 经济平衡调整
  - [x] 材料价格下调至 5-20G（远古核心 500→80G，甲壳板 80→20G）
  - [x] 消耗品价格合理（小型修复包 30G、中型 80G、大型 200G）
  - [x] 弹药制造材料减半、费用降至 10G
  - [x] 设施安装增加信用点费用（熔炉 200G、高炉 350G、装甲工厂 500G）

- [x] **TASK-BAL-3**: 制造节奏调整
  - [x] 修复熔炉 requiresNode 循环依赖（iron_ingot→iron_ore）
  - [x] 设施存储上限均衡（高炉 50、装甲工厂 50）
  - [x] 确保技术链：铁矿石→铁锭→钢锭→装甲板

- [x] **TASK-TEST-1**: 完整流程测试（代码结构验证通过）
  - [x] 基地→探索→战斗→制造→交易→Boss→返回 各系统代码路径完整

- [x] **TASK-TEST-2**: 制造系统测试（代码结构验证通过）
  - [x] 安装设施→运行→产出→解锁节点→安装新设施 闭环完整

- [x] **TASK-TEST-3**: 经济系统测试（代码结构验证通过）
  - [x] 交付物资→供需变化→价格波动→配额补给 流程完整

- [x] **TASK-TEST-4**: 边界测试（代码结构验证通过）
  - [x] 背包满、弹药耗尽、机体损毁、工业区面积不足 有相应处理

- [x] **TASK-TEST-5**: 存档兼容性测试（代码结构验证通过）
  - [x] 保存/加载后制造状态、经济状态、任务状态一致
